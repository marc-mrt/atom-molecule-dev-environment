...
export default {
  ...
  getSuggestions(options: {
    ...
    if (uri == null) {
      console.log(
        "Cannot provide completion for file",
        options.editor.getTitle(),
        uri,
      );
      return [];
    }
    ...
    return eventEmitter.emit("textDocument/completion", params);

    return new Promise(
      (resolve): any => {
        const data = eventEmitter.emit("textDocument/completion", params);
        console.log(data);
        resolve(data);
      },
    );
